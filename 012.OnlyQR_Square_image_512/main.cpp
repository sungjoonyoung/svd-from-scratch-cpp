...
int main(void){
   ...
   ifstream fin;fin.open("512_Mandrill.bmp",ios::binary);
   A=image_func(fin);
   ...
   while(1){
      ...
      if(in==1){
         ...
         op=min(max(0,op),N-1);
         auto re=mat_zero(A.size());
         re=mat_plus(re,vecvec_to_mat(big_U[op],big_V[op]));
         // for(int i=0;i<N;i++)for(int j=0;j<N;j++)re[i][j]*=128.0;
         ofstream fout;fout.open("tmp.bmp",ios::binary);
         saveBMP(fout,re);
         cout<<"\n완료! tmp.bmp를 확인해보세요\n";
      }
      else if(in==2){
